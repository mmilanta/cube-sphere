...
export class IsometricCanvas{
    ...
    _render_block(block_shape: BlockShape, block_position: Coords3d){
        ...
        for (const path of paths){
            ...
            this.context.moveTo((path[0][0][0] * this.ratio) + offset[0], (path[0][0][1] * this.ratio) + offset[1]);
            for (let i = 1; i < path.length; i++) {
                this.context.lineTo((path[0][i][0] * this.ratio) + offset[0], (path[0][i][1] * this.ratio) + offset[1]);
            }
            this.context.fillStyle = this.colors[path[1]];
            this.context.fill();
